...
# HideSeek
async def fetch_hide_stats(player_name):
    api_url = f"https://api.playhive.com/v0/game/all/hide/{player_name}"
    profile_url = f"https://hivebe.link/p/{player_name}"
    ...
    try:
        ...
        stats = {
            ...
            "อัตราการชนะ": f"{win_rate:.2f}%",
            "จำนวนการตาย": deaths,
            "กำจัดคนหาไปแล้ว": api_data.get("seeker_kills", 0),
            "เจอคนซ่อนไปแล้ว": api_data.get("hider_kills", 0),
            "XP": f"{xp} (LV.{level})",
            "เริ่มเล่นครั้งแรกเมื่อ": first_played,
        }
...
#Drop
async def fetch_drop_stats(player_name):
    api_url = f"https://api.playhive.com/v0/game/all/drop/{player_name}"
    profile_url = f"https://hivebe.link/p/{player_name}"
    ...
    try:
        ...
        stats = {
            ...
            "อัตราการชนะ": f"{win_rate:.2f}%",
            "จำนวนการตาย": deaths,
            "บล็อคท่ทำลาย": api_data.get("blocks_destroyed", 0),
            "รวมรวมพลังเสริม": api_data.get("powerups_collected", 0),
            "ใช้สกิลพุ่งตัวไปแล้ว": api_data.get("vaults_used", 0),
            "XP": f"{xp} (LV.{level})",
            "เริ่มเล่นครั้งแรกเมื่อ": first_played,
        }
...
#groundwar
async def fetch_ground_stats(player_name):
    api_url = f"https://api.playhive.com/v0/game/all/ground/{player_name}"
    profile_url = f"https://hivebe.link/p/{player_name}"
    ...
    try:
        ...
        stats = {
            ...
            "K/D": f"{kd_ratio:.2f}",
            "จำนวนฆ่าต่อเกม": f"{kills_per_game:.2f}",
            "จำนวนบล็อคที่วาง": api_data.get("blocks_placed", 0),
            "จำนวนบล็อคที่ทำลาย": api_data.get("blocks_destroyed", 0),
            "ยิงวัตถุไปแล้ว": api_data.get("projectiles_fired", 0),
            "XP": f"{xp} (LV.{level})",
            "เริ่มเล่นครั้งแรกเมื่อ": first_played,
            "สไตล์การเล่น": play_style  # เพิ่มข้อมูลสไตล์การเล่น
        }
...
#party
async def fetch_party_stats(player_name):
    api_url = f"https://api.playhive.com/v0/game/all/party/{player_name}"
    profile_url = f"https://hivebe.link/p/{player_name}"
    ...
    try:
        ...
        stats = {
            ...
            "เกมที่ชนะ": victories,
            "อัตราการชนะ": f"{win_rate:.2f}%",
            "รอบที่เอาตัวรอดได้": api_data.get("rounds_survived", 0),
            "รวมรวมพลังเสริม": api_data.get("powerups_collected", 0),
            "XP": f"{xp} (LV.{level})",
            "เริ่มเล่นครั้งแรกเมื่อ": first_played,
        }
...
#bridge
async def fetch_bridge_stats(player_name):
    api_url = f"https://api.playhive.com/v0/game/all/bridge/{player_name}"
    profile_url = f"https://hivebe.link/p/{player_name}"
    ...
    try:
        ...
        stats = {
            ...
            "K/D": f"{kd_ratio:.2f}",
            "จำนวนฆ่าต่อเกม": f"{kills_per_game:.2f}",
            "เข้าหลุมอีกฝั่งไปแล้ว": api_data.get("goals", 0),
            "XP": f"{xp} (LV.{level})",
            "เริ่มเล่นครั้งแรกเมื่อ": first_played,
            "ความถนัดในการเล่น": play_style  # เพิ่มข้อมูลสไตล์การเล่น
        }
...
#gravity
async def fetch_gravity_stats(player_name):
    api_url = f"https://api.playhive.com/v0/game/all/grav/{player_name}"
    profile_url = f"https://hivebe.link/p/{player_name}"
    ...
    try:
        ...
        stats = {
            ...
            "อัตราการชนะ": f"{win_rate:.2f}%",
            "จำนวนการตาย": deaths,
            "ผ่านด่านแล้ว": api_data.get("maps_completed", 0),
            "ผ่านด่านโดยที่ไม่ตายแล้ว": api_data.get("maps_completed_without_dying", 0),
            "XP": f"{xp} (LV.{level})",
            "เริ่มเล่นครั้งแรกเมื่อ": first_played,
        }
